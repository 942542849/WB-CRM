...
<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
  ...
  <div class="layui-btn-container">
    <div class="layui-input-inline">
      <input type="text" name="txt" lay-verify="title"  autocomplete="off" placeholder="请输入销售机会描述" class="layui-input input">
    </div>
    <button class="layui-btn layui-btn-sm" lay-event="search">查询机会描述</button>
    <button class="layui-btn layui-btn-sm" lay-event="add">新增机会描述</button>
  </div>
</script>
		...
		layui.use('table', function() {
			...
			table.on('tool(test)', function(obj) {
				...
				if (obj.event === 'del') { ///lay-event 属性
					...
					openFrame('Oppstate_edit.jsp?id='+data.id);
				}
			});
